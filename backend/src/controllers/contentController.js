...
const { cached, del } = require("../utils/cache");

async function getAll(req, res) {
  const data = await cached("content:all", 30, async () => {
...
async function getById(req, res) {
  ...
  // Increment views (fire-and-forget so it doesn't slow the response)
  sb.from("content").update({ views: (data.views || 0) + 1 }).eq("id", req.params.id).then(() => {
    del("content:"); // invalidate cached lists since view counts changed ordering
  });
  return ok(res, data);
}
...
async function search(req, res) {
  const q = req.query.q || "";
  const data = await cached(`content:search:${q}`, 30, async () => {
...
async function getTrending(req, res) {
  const data = await cached("content:trending", 30, async () => {
