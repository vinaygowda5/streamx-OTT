...
function Btn({children,onClick,color=R,outline,danger,small,disabled,loading:ld,style:sx={}}){
  return <button disabled={disabled||ld} onClick={onClick} style={{background:danger?"rgba(248,113,113,.1)":outline?`${color}18`:color,border:`1px solid ${danger?"rgba(248,113,113,.3)":outline?color+"55":"transparent"}`,color:outline||danger?(danger?"#f87171":color):"#fff",borderRadius:7,padding:small?"4px 10px":"7px 14px",fontSize:small?11:12.5,fontWeight:600,cursor:disabled||ld?"not-allowed":"pointer",fontFamily:"'Inter',sans-serif",transition:"all .15s",whiteSpace:"nowrap",opacity:disabled||ld?.6:1,...sx}}>{ld?"...":children}</button>;
}
...
/* ── SMALL SCREEN VIDEO PREVIEW ─────────────────
   Paste any URL → preview appears instantly
   Supports: HLS .m3u8, MP4, YouTube, embed links
──────────────────────────────────────────────── */
function SmallPreview({url,isLive=false}){
  ...
  const isEmbed = !!(url?.includes("youtube.com/embed/")||url?.includes("twitch.tv")||url?.includes("facebook.com/plugins/video")||url?.includes("dailymotion.com/embed"));
  ...
  useEffect(()=>{
    if(!url) return;
    if(isYT||isEmbed){setStatus("playing");return;}
  ...
  return(
    <div style={{borderRadius:10,overflow:"hidden",background:"#000",border:"1px solid #1a1a26",marginTop:10}}>
      {/* 16:9 box */}
      <div style={{paddingTop:"56.25%",position:"relative",background:"#000"}}>
          ...
          <>
            ...
            {status==="loading"&&(
              <div style={{position:"absolute",inset:0,display:"flex",alignItems:"center",justifyContent:"center",background:"#000"}}>
                <div style={{width:28,height:28,border:"2px solid #222",borderTop:`2px solid ${R}`,borderRadius:"50%",animation:"spin .8s linear infinite"}}/>
              </div>
            )}
            {status==="error"&&(
              <div style={{position:"absolute",inset:0,display:"flex",alignItems:"center",justifyContent:"center",background:"#0a0a0f",flexDirection:"column",gap:8}}>
                <div style={{fontSize:28}}>⚠️</div>
                <div style={{fontSize:11,color:"#f87171",textAlign:"center",padding:"0 12px"}}>Stream error. Check URL.</div>
              </div>
            )}
          </>
        )}
        ...
        {/* Muted notice */}
        {status==="playing"&&!isYT&&!isEmbed&&<div style={{position:"absolute",bottom:8,right:8,background:"rgba(0,0,0,.7)",color:"#aaa",fontSize:10,padding:"2px 7px",borderRadius:4,zIndex:10}}>🔇 Preview</div>}
      </div>
      {/* Status bar */}
      <div style={{background:"#0a0a14",padding:"6px 12px",display:"flex",alignItems:"center",gap:8}}>
        <div style={{width:7,height:7,borderRadius:"50%",background:status==="playing"?"#00c853":status==="error"?"#f87171":"#f59e0b",flexShrink:0,animation:status==="playing"?"pulse 2s infinite":"none"}}/>
        <div style={{fontSize:11,color:status==="playing"?"#00c853":status==="error"?"#f87171":"#f59e0b",fontWeight:600}}>
          {status==="playing"?"Stream OK — Preview Live":status==="error"?"Stream Error":"Connecting..."}
        </div>
      </div>
    </div>
  );
    ...
    <div style={{display:"flex",flexDirection:"column",gap:14}}>
      ...
      <div style={{display:"grid",gridTemplateColumns:"2fr 1fr",gap:12}}>
        <Field label="Title" required>
          <input className="inp" value={form.title} onChange={e=>set("title",e.target.value)} placeholder={isLiveForm?"Channel name e.g. IPL Live":"Movie or series title"} autoFocus/>
        </Field>
        {!isLiveForm&&(
          <Field label="Type">
      ...
      <div style={{background:"rgba(229,9,20,.05)",border:"1px solid rgba(229,9,20,.15)",borderRadius:12,padding:16}}>
        ...
        {/* URL tab */}
        {srcTab==="url"&&(
          <Field label={isLiveForm?"HLS Live Stream URL (.m3u8)":"HLS / MP4 / DASH URL"} hint="Supports: .m3u8, .mp4, .mpd — direct streaming links">
        ...
        {/* YouTube tab */}
        {srcTab==="youtube"&&(
          <Field label="YouTube URL" hint="Paste any YouTube video or live stream — auto converted to embed">
            <input className="inp" value={form.embed_url} onChange={e=>handleUrlChange(e.target.value,"embed_url")} placeholder="https://youtube.com/watch?v=XXXXXXXXXX  or  youtu.be/XXXX"/>
          </Field>
        )}
            ...
            <div className="upload-zone" onClick={()=>fileRef.current?.click()}>
              ...
              {uploading?(
                ...
                <div>
                  ...
                  <div style={{fontSize:13,color:"#aaa",fontWeight:600,marginBottom:4}}>Click to upload video from your device</div>
        ...
        {srcTab==="drive"&&(
          <div style={{display:"flex",flexDirection:"column",gap:10}}>
            <Field label="Paste any video link" hint="Google Drive, Dropbox, OneDrive, Cloudflare R2, Bunny CDN, etc.">
            ...
            <div style={{background:"rgba(255,255,255,.03)",borderRadius:8,padding:10,fontSize:11,color:"#555",lineHeight:1.7}}>
              <span style={{color:"#1d9bf0"}}>Google Drive:</span> Share → Anyone with link → Copy → change "view" to "uc?export=download"<br/>
              <span style={{color:"#00c853"}}>Dropbox:</span> Share → change "dl=0" to "dl=1"<br/>
              <span style={{color:"#f59e0b"}}>Cloudflare R2:</span> Copy public URL directly (fastest option)
            </div>
          </div>
        )}
        ...
        {previewUrl && previewUrl.length > 10 && (
          <div>
            <div style={{fontSize:11,color:"#555",fontWeight:700,textTransform:"uppercase",letterSpacing:.5,margin:"12px 0 6px"}}>Live Preview</div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,padding:"10px 0",borderTop:"1px solid #191926"}}>
        {(isLiveForm?[
          ["is_active","✅ Active (show to users)"],
...
/* ── SmallPreview component (used inside ContentForm above) ── */
function SmallPreview({url,isLive=false}){
  const videoRef=useRef(null);
  const hlsRef  =useRef(null);
  const[status,setStatus]=useState("loading");
  const isYT=url?.includes("youtube.com")||url?.includes("youtu.be");
  function toEmbed(u){try{if(u.includes("youtube.com/watch?v=")){const id=new URL(u).searchParams.get("v");return`https://www.youtube.com/embed/${id}?autoplay=1&mute=1&rel=0`;}if(u.includes("youtu.be/")){const id=u.split("youtu.be/")[1]?.split("?")[0];return`https://www.youtube.com/embed/${id}?autoplay=1&mute=1&rel=0`;}}catch(e){}return u;}
  useEffect(()=>{
    if(!url) return;
    ...
    try{
      if(Hls.isSupported()){
        ...
        hls.on(Hls.Events.MANIFEST_PARSED,()=>{v.muted=true;v.play().catch(()=>{});setStatus("playing");});
        hls.on(Hls.Events.ERROR,(_,d)=>{if(d.fatal) setStatus("error");});
      }else if(v.canPlayType("application/vnd.apple.mpegurl")){v.src=url;v.muted=true;v.play().catch(()=>{});setStatus("playing");
      }else{v.src=url;v.muted=true;v.play().catch(()=>{});setStatus("playing");}
    }catch(e){setStatus("error");}
    return()=>{if(hlsRef.current){hlsRef.current.destroy();hlsRef.current=null;}};
  },[url]);
  return(
  ...
  return(
    <div style={{animation:"fadeIn .3s ease"}}>
      {confirm&&(
        <Modal title="Confirm Delete" onClose={()=>setConfirm(null)}>
          <div style={{textAlign:"center",padding:"8px 0"}}>
            ...
            <div style={{fontSize:13,color:"#666",marginBottom:24}}>Permanently removes from database. Cannot undo.</div>
      ...
      <div style={{marginBottom:12}}>
        <input className="inp" value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search..." style={{maxWidth:280}}/>
      </div>
      <div className="card" style={{overflow:"hidden"}}>
        {items.length===0?(
          ...
          <div style={{overflowX:"auto"}}>
            <table className="tbl">
              <thead><tr><th>Title</th><th>Type/Genre</th><th>Status</th><th>Views</th><th>Actions</th></tr></thead>
              <tbody>
                {items.map(c=>(
                  <tr key={c.id}>
                    <td>
                    ...
                    <td><Chip label={c.type||"Movie"} color="#1d9bf0"/><span style={{marginLeft:6,fontSize:11,color:"#555"}}>{c.genre}</span></td>
                    <td><div style={{display:"flex",gap:4,flexWrap:"wrap"}}><Chip label={c.is_active?"ON":"OFF"} color={c.is_active?"#00c853":"#555"}/>{c.is_featured&&<Chip label="⭐" color="#f59e0b"/>}{c.is_premium&&<Chip label="👑" color={R}/>}{c.is_live&&<Chip label="🔴" color={R}/>}</div></td>
                    ...
                    <td>
                      <div style={{display:"flex",gap:4}}>
                        ...
                        <Btn onClick={async()=>{await db.updateContent(c.id,{is_featured:!c.is_featured});showToast(c.is_featured?"Removed from featured":"Featured ⭐");onRefresh();}} outline small color="#f59e0b">{c.is_featured?"★":"☆"}</Btn>
  ...
  return(
    <div style={{animation:"fadeIn .3s ease"}}>
      ...
      <div className="card" style={{overflow:"hidden"}}>
        <div style={{overflowX:"auto"}}>
          <table className="tbl">
            <thead><tr><th>User</th><th>Phone</th><th>Plan</th><th>Role</th><th>Status</th><th>Joined</th><th>Action</th></tr></thead>
            <tbody>
              {filtered.map(u=>(
                <tr key={u.id}>
      ...
      {modal!==null&&(
        <Modal title={modal?.id?"Edit Ad":"New Ad"} onClose={()=>setModal(null)}>
          <div style={{display:"flex",flexDirection:"column",gap:12}}>
            {[["Brand","brand"],["Tagline","tagline"],["CTA Text","cta_text"],["Video URL (optional)","video_url"]].map(([l,k])=>(
      ...
      <div style={{flex:1,display:"flex",flexDirection:"column",minWidth:0,overflow:"hidden"}}>
        <div style={{height:48,background:"#0a0a14",borderBottom:"1px solid #191926",display:"flex",alignItems:"center",padding:"0 20px",gap:12,flexShrink:0}}>
          ...
          <div style={{display:"flex",alignItems:"center",gap:6,fontSize:11,color:"#444"}}>
            <div style={{width:6,height:6,borderRadius:"50%",background:"#00c853"}}/>Secured
          </div>
          ...
          {page==="settings" &&(
            <div style={{animation:"fadeIn .3s ease",maxWidth:560}}>
              ...
              <div className="card" style={{padding:20,marginBottom:14}}>
                {[["Admin Phone","+918660570052"],["Database","Supabase PostgreSQL"],["Auth","Mobile OTP"],["Version","StreamX v3.0"],["Frontend","Vercel"],["Backend","Railway"]].map(([k,v])=>(
