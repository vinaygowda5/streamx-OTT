...
function StreamPreview({url,isLive=false}){
  const vRef=useRef(null); const hlsRef=useRef(null);
  ...
  useEffect(()=>{
    if(!url)return; if(isYT){setSt("playing");return;}
    const v=vRef.current;if(!v)return; setSt("loading");
    try{
      if(Hls.isSupported()){
  ...
  const stColor=st==="playing"?GR:st==="error"?"#f87171":AM;
  const stLabel=st==="playing"?"✓ Stream working":st==="error"?"✗ Error — check URL":"Connecting...";
  return(
    <div style={{borderRadius:12,overflow:"hidden",background:"#000",border:"1px solid #181828",marginTop:12}}>
      <div style={{paddingTop:"56.25%",position:"relative"}}>
        ...
        :<><video ref={vRef} style={{position:"absolute",inset:0,width:"100%",height:"100%",objectFit:"contain"}} playsInline muted autoPlay controls/>
          {st==="loading"&&<div style={{position:"absolute",inset:0,display:"flex",alignItems:"center",justifyContent:"center",background:"rgba(0,0,0,.8)"}}><div style={{width:36,height:36,border:`2px solid #181828`,borderTop:`2px solid ${R}`,borderRadius:"50%",animation:"spin .8s linear infinite"}}/></div>}
          {st==="error"&&<div style={{position:"absolute",inset:0,display:"flex",alignItems:"center",justifyContent:"center",background:"rgba(0,0,0,.9)",flexDirection:"column",gap:10}}><span style={{fontSize:32}}>⚠️</span><div style={{fontSize:12,color:"#f87171"}}>Stream unavailable</div></div>}
        </>}
    ...
    <div style={{display:"flex",height:"100vh",background:"#04040e",overflow:"hidden",fontFamily:"'Inter',sans-serif"}}>
      ...
      <div style={{width:collapsed?56:210,flexShrink:0,background:"linear-gradient(180deg,#080814 0%,#060610 100%)",borderRight:"1px solid #181828",display:"flex",flexDirection:"column",transition:"width .22s ease",overflow:"hidden",boxShadow:"4px 0 24px rgba(0,0,0,.4)"}}>
        ...
        <div style={{padding:collapsed?"14px 12px":"14px 18px",borderBottom:"1px solid #181828",display:"flex",alignItems:"center",gap:12,cursor:"pointer",flexShrink:0}} onClick={()=>setCollapsed(o=>!o)}>
          ...
          {!collapsed&&<div style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:18,letterSpacing:2,whiteSpace:"nowrap"}}><span style={{color:R}}>STREAM</span><span style={{color:"#e2e2f0"}}>X</span><span style={{fontSize:9,color:"#3a3a5a",marginLeft:8,fontFamily:"Inter",letterSpacing:0}}>ADMIN PRO</span></div>}
        </div>
