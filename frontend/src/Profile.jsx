...
export default function Profile({ onNavigate, user, onLogout, onUpgrade }) {
  ...
  useEffect(()=>{
    ...
    // Load saved prefs
    try {
      const saved = localStorage.getItem("nammacinema_prefs_"+user.id);
      if(saved) setPrefs(JSON.parse(saved));
      const savedLang = localStorage.getItem("nammacinema_lang_"+user.id);
  ...
  async function loadData(){
    try {
      ...
      setWatchlist(w||[]); setHistory(h||[]);
      setNotifs(n||[]); setSub(s||null);
    } catch(e){}
  }
  ...
  async function saveName(){
    ...
    setSavingName(true);
    try {
      const u = await db.updateUser(user.id,{name:newName.trim()});
      setUserData(u||{...userData,name:newName.trim()});
      localStorage.setItem("nammacinema_user",JSON.stringify(u||userData));
      showToast("Name updated ✓");
      setEditName(false);
    } catch(e){ showToast("Failed to update name","err"); }
    setSavingName(false);
  }
  ...
  async function saveEmail(){
    ...
    if(!newEmail.trim()){
      ...
      setPrefs(p=>({...p,emailAlerts:false}));
      savePrefs({...prefs,emailAlerts:false});
      try { await db.updateUser(user.id,{email:""}); } catch(e){}
      setUserData(u=>({...u,email:""}));
      localStorage.setItem("nammacinema_user",JSON.stringify({...userData,email:""}));
    ...
    setSavingEmail(true);
    try {
      const u = await db.updateUser(user.id,{email:newEmail.trim()});
      setUserData(u||{...userData,email:newEmail.trim()});
      localStorage.setItem("nammacinema_user",JSON.stringify(u||userData));
      // Auto enable email alerts when email is added
      const newPrefs = {...prefs,emailAlerts:true};
      setPrefs(newPrefs);
      savePrefs(newPrefs);
      showToast("Email saved! Alerts enabled ✓");
      setEditEmail(false);
    } catch(e){ showToast("Failed to save email","err"); }
    setSavingEmail(false);
  }

  function savePrefs(p){
    if(!user?.id) return;
    localStorage.setItem("nammacinema_prefs_"+user.id, JSON.stringify(p));
  }

  function togglePref(key){
    // Email alerts require email first
    if(key==="emailAlerts" && !prefs.emailAlerts && !userData?.email){
      showToast("Add your email address first","err");
      setEditEmail(true);
      return;
    }
    const n = {...prefs,[key]:!prefs[key]};
    setPrefs(n); savePrefs(n);
    showToast(n[key]?"Enabled ✓":"Disabled");
  }

  function saveLang(code){
    setAppLang(code);
    if(user?.id) localStorage.setItem("nammacinema_lang_"+user.id, code);
    db.updateUser(user.id,{language:code}).catch(()=>{});
  ...
  const Card = ({children,style={}}) => (
    <div style={{background:S1,border:`1px solid ${BD}`,borderRadius:14,padding:16,marginBottom:14,...style}}>
      {children}
    </div>
  );

  const Row = ({icon,label,sub,right,onClick,last,danger}) => (
    <div onClick={onClick} style={{display:"flex",alignItems:"center",gap:12,padding:"11px 0",borderBottom:last?"none":`1px solid ${BD}22`,cursor:onClick?"pointer":"default",transition:"opacity .15s"}} onMouseEnter={e=>{if(onClick)e.currentTarget.style.opacity=".8";}} onMouseLeave={e=>e.currentTarget.style.opacity="1"}>
  ...
  return (
    <div style={{minHeight:"100vh",background:BG,fontFamily:"Inter,sans-serif",paddingBottom:80}}>
      ...
      <div style={{maxWidth:580,margin:"0 auto",padding:"16px 16px 40px"}}>
          ...
          <div style={{animation:"fadeUp .3s ease"}}>
            ...
            {editEmail && (
              <Card>
                ...
                <div style={{fontSize:12,color:MT,marginBottom:10}}>Required for email alerts and notifications</div>
        ...
        {tab==="history" && (
          <div>
            <div style={{display:"flex",justifyContent:"space-between",marginBottom:12}}>
              ...
              {history.length>0 && <button onClick={async()=>{await db.clearHistory(user.id).catch(()=>{});setHistory([]);showToast("History cleared");}} style={{background:"none",border:"none",color:"#f87171",fontSize:12,fontWeight:600,cursor:"pointer"}}>Clear All</button>}
            </div>
            <Card>
              {history.length===0 ? (
                <div style={{textAlign:"center",padding:"32px 0",color:MT}}>
                ...
                const c=item.content||{};
                return (
                  <div key={item.id} style={{display:"flex",alignItems:"center",gap:12,padding:"10px 0",borderBottom:i<history.length-1?`1px solid ${BD}22`:"none"}}>
                    <div style={{width:48,height:32,borderRadius:6,background:"linear-gradient(135deg,#1d9bf022,#0a0a0f)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:16,flexShrink:0,position:"relative",overflow:"hidden"}}>
                      🎬
                      {(item.progress_pct||0)>0 && <div style={{position:"absolute",bottom:0,left:0,width:`${item.progress_pct}%`,height:2,background:RED}}/>}
                    </div>
                    <div style={{flex:1,minWidth:0}}>
                      <div style={{fontWeight:600,fontSize:13,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{c.title||"Unknown"}</div>
                      <div style={{fontSize:11,color:MT}}>{item.progress_pct||0}% watched</div>
                    </div>
                    <div style={{fontSize:11,color:MT}}>{new Date(item.watched_at).toLocaleDateString("en-IN")}</div>
                  </div>
                );
              })}
            </Card>
        ...
        {tab==="notifications" && (
          <div>
            <div style={{display:"flex",justifyContent:"space-between",marginBottom:12}}>
              ...
              {unread>0 && <button onClick={async()=>{await db.markAllNotifsRead(user.id).catch(()=>{});setNotifs(n=>n.map(x=>({...x,is_read:true})));}} style={{background:"none",border:"none",color:RED,fontSize:12,fontWeight:600,cursor:"pointer"}}>Mark all read</button>}
            </div>
            <Card>
              {notifs.length===0 ? (
                <div style={{textAlign:"center",padding:"32px 0",color:MT}}>
                ...
                <div key={n.id} onClick={()=>db.markNotifRead(n.id).then(()=>setNotifs(p=>p.map(x=>x.id===n.id?{...x,is_read:true}:x))).catch(()=>{})}
                  style={{display:"flex",gap:12,padding:"11px 0",borderBottom:i<notifs.length-1?`1px solid ${BD}22`:"none",cursor:"pointer",opacity:n.is_read?.5:1}}>
                  <div style={{width:34,height:34,borderRadius:9,background:n.is_read?"rgba(255,255,255,.04)":`${RED}18`,border:`1px solid ${n.is_read?BD:RED+"44"}`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:15,flexShrink:0}}>
                    {n.type==="welcome"?"🎉":"🔔"}
                  </div>
                  <div style={{flex:1}}>
                    <div style={{fontWeight:n.is_read?400:700,fontSize:13}}>{n.title}</div>
                    <div style={{fontSize:11,color:MT,marginTop:2}}>{n.message}</div>
                  </div>
        ...
        {tab==="settings" && (
          <div style={{animation:"fadeUp .3s ease"}}>
            ...
            {/* Preferences */}
            <Card>
              <div style={{fontSize:11,color:MT,fontWeight:700,textTransform:"uppercase",letterSpacing:.5,marginBottom:10}}>Preferences</div>
              {[
                ["▶️","Autoplay","Auto-play next episode","autoplay"],
                ["⏭️","Skip Intro","Skip opening credits automatically","skipIntro"],
                ["🔔","Push Notifications","Alerts for new content","notifications"],
              ].map(([icon,label,sub,key])=>(
                <div key={key} style={{display:"flex",alignItems:"center",gap:12,padding:"10px 0",borderBottom:`1px solid ${BD}22`}}>
                  <div style={{width:34,height:34,borderRadius:9,background:"rgba(255,255,255,.05)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:16,flexShrink:0}}>{icon}</div>
                  <div style={{flex:1}}>
                    <div style={{fontSize:13,fontWeight:600,color:"#e0e0ee"}}>{label}</div>
                    <div style={{fontSize:11,color:MT}}>{sub}</div>
                  </div>
                  <Toggle on={prefs[key]} onChange={()=>togglePref(key)}/>
                </div>
              ))}
              {/* Email alerts — requires email */}
              <div style={{display:"flex",alignItems:"center",gap:12,padding:"10px 0"}}>
                <div style={{width:34,height:34,borderRadius:9,background:"rgba(255,255,255,.05)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:16,flexShrink:0}}>📧</div>
                <div style={{flex:1}}>
                  <div style={{fontSize:13,fontWeight:600,color:"#e0e0ee"}}>Email Alerts</div>
                  <div style={{fontSize:11,color:MT}}>
                    {userData?.email ? `Sending to ${userData.email}` : "Add email to enable alerts"}
                  </div>
                  {!userData?.email && (
                    <button onClick={()=>{setTab("profile");setEditEmail(true);setNewEmail("");}} style={{background:"none",border:"none",color:RED,fontSize:11,fontWeight:700,cursor:"pointer",fontFamily:"Inter,sans-serif",padding:0,marginTop:3}}>
                      + Add Email →
                    </button>
                  )}
                </div>
                <Toggle on={prefs.emailAlerts && !!userData?.email} onChange={()=>togglePref("emailAlerts")}/>
              </div>
            </Card>

            {/* Language */}
            <Card>
              <div style={{fontSize:11,color:MT,fontWeight:700,textTransform:"uppercase",letterSpacing:.5,marginBottom:10}}>App Language</div>
              <div onClick={()=>setShowLang(true)} style={{display:"flex",alignItems:"center",gap:12,cursor:"pointer",padding:"4px 0"}}>
                <div style={{width:34,height:34,borderRadius:9,background:"rgba(255,255,255,.05)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:18,flexShrink:0}}>🌐</div>
                <div style={{flex:1}}>
                  <div style={{fontSize:13,fontWeight:600}}>{selLang.flag} {selLang.label}</div>
                  <div style={{fontSize:11,color:MT}}>Tap to change language</div>
                </div>
            ...
            {showLang && (
              <div style={{position:"fixed",inset:0,zIndex:900,background:"rgba(0,0,0,.85)",display:"flex",alignItems:"flex-end",justifyContent:"center"}} onClick={()=>setShowLang(false)}>
                <div style={{background:"#111120",borderRadius:"16px 16px 0 0",width:"100%",maxWidth:500,maxHeight:"70vh",overflow:"auto",padding:"0 0 24px",animation:"fadeUp .25s ease"}} onClick={e=>e.stopPropagation()}>
                  <div style={{display:"flex",justifyContent:"center",padding:"12px 0 6px"}}><div style={{width:40,height:4,borderRadius:2,background:"#333"}}/></div>
                  <div style={{fontWeight:700,fontSize:16,padding:"8px 20px 12px",borderBottom:`1px solid ${BD}`}}>Select Language</div>
                  {LANGUAGES.map(l=>(
                    <div key={l.code} className="lang-opt" onClick={()=>saveLang(l.code)}>
                      <span style={{fontSize:22}}>{l.flag}</span>
                      <span style={{flex:1,fontSize:14,fontWeight:appLang===l.code?700:400,color:appLang===l.code?"#fff":"#aaa"}}>{l.label}</span>
                      {appLang===l.code && <span style={{color:RED,fontWeight:800}}>✓</span>}
                    </div>
                  ))}
                </div>
              </div>
            ...
            {/* Privacy */}
            <Card>
              <div style={{fontSize:11,color:MT,fontWeight:700,textTransform:"uppercase",letterSpacing:.5,marginBottom:10}}>Privacy</div>
              <Row icon="👁️" label="Watch History" sub="Control what you've watched" onClick={()=>setPrivacySection("history")}/>
              <Row icon="📊" label="Data & Personalisation" sub="How we use your data" onClick={()=>setPrivacySection("data")}/>
              <Row icon="🔒" label="Account Security" sub="Keep your account safe" onClick={()=>setPrivacySection("security")} last/>
            </Card>

            {/* Privacy detail modal */}
            {privacySection && (
              <div style={{position:"fixed",inset:0,zIndex:900,background:"rgba(0,0,0,.85)",display:"flex",alignItems:"center",justifyContent:"center",padding:20}} onClick={()=>setPrivacySection(null)}>
                <div style={{background:"#111120",border:`1px solid ${BD}`,borderRadius:16,width:"100%",maxWidth:420,padding:24,animation:"fadeIn .2s ease"}} onClick={e=>e.stopPropagation()}>
                  {privacySection==="history" && <>
                    <div style={{fontWeight:700,fontSize:17,marginBottom:14}}>👁️ Watch History</div>
                    <div style={{fontSize:13,color:"#aaa",lineHeight:1.7,marginBottom:20}}>Your watch history helps us recommend content you'll love. You can clear your history anytime from the History tab.</div>
                    <button onClick={async()=>{await db.clearHistory(user.id).catch(()=>{});setHistory([]);showToast("Watch history cleared");setPrivacySection(null);}} style={{width:"100%",background:"rgba(248,113,113,.1)",border:"1px solid rgba(248,113,113,.3)",color:"#f87171",borderRadius:9,padding:"11px",fontWeight:700,fontSize:13,cursor:"pointer",fontFamily:"Inter,sans-serif",marginBottom:8}}>🗑️ Clear Watch History</button>
                    <button onClick={()=>setPrivacySection(null)} style={{width:"100%",background:"rgba(255,255,255,.06)",border:`1px solid ${BD}`,color:"#aaa",borderRadius:9,padding:"11px",fontSize:13,cursor:"pointer",fontFamily:"Inter,sans-serif"}}>Close</button>
                  </>}
                  {privacySection==="data" && <>
                    <div style={{fontWeight:700,fontSize:17,marginBottom:14}}>📊 Data & Personalisation</div>
                    <div style={{fontSize:13,color:"#aaa",lineHeight:1.7,marginBottom:20}}>Namma Cinema collects your watch history and preferences to personalise your experience. We never sell your data to third parties. Your data is stored securely on Supabase servers.</div>
                    <div style={{background:"rgba(255,255,255,.04)",borderRadius:9,padding:12,marginBottom:16}}>
                      {[["Watch History","Stored for recommendations"],["Preferences","Stored locally on device"],["Phone Number","Used for login only"],["Payment Data","Processed by Razorpay"]].map(([k,v])=>(
                        <div key={k} style={{display:"flex",justifyContent:"space-between",padding:"6px 0",fontSize:12,borderBottom:`1px solid ${BD}22`}}>
                  ...
                  {privacySection==="security" && <>
                    <div style={{fontWeight:700,fontSize:17,marginBottom:14}}>🔒 Account Security</div>
                    <div style={{fontSize:13,color:"#aaa",lineHeight:1.7,marginBottom:16}}>Your account is protected with OTP-based authentication. No password is stored.</div>
                    {[["✅","OTP Login","Active — secured via Supabase"],["✅","Phone Verified",userData?.phone||"Not set"],["📧","Email",userData?.email||"Not added — tap to add"]].map(([icon,label,val])=>(
                      <div key={label} style={{display:"flex",gap:10,alignItems:"center",padding:"8px 0",borderBottom:`1px solid ${BD}22`}}>
                        <span style={{fontSize:16}}>{icon}</span>
                        <div style={{flex:1}}><div style={{fontSize:13,fontWeight:600}}>{label}</div><div style={{fontSize:11,color:MT}}>{val}</div></div>
                      </div>
                    ))}
                    <button onClick={()=>setPrivacySection(null)} style={{width:"100%",background:"rgba(255,255,255,.06)",border:`1px solid ${BD}`,color:"#aaa",borderRadius:9,padding:"11px",fontSize:13,cursor:"pointer",fontFamily:"Inter,sans-serif",marginTop:16}}>Close</button>
                  </>}
                </div>
              </div>
            )}
            ...
            <Card>
              ...
              <Row icon="📋" label="Terms of Use" sub="Namma Cinema terms and conditions" onClick={()=>window.open("https://nammacinema-ott.vercel.app","_blank")}/>
              <Row icon="🛡️" label="Privacy Policy" sub="How we protect your data" onClick={()=>setPrivacySection("data")} last/>
            </Card>

            {/* Sign out */}
            <button onClick={onLogout} style={{width:"100%",background:"rgba(255,255,255,.04)",border:`1px solid ${BD}`,color:"#aaa",borderRadius:12,padding:"13px",fontFamily:"Inter,sans-serif",fontSize:14,fontWeight:600,cursor:"pointer",marginBottom:12}}>
              Sign Out
            </button>

            {/* Delete account */}
            <button onClick={()=>{setShowDelete(true);setDeleteStep(1);setDeleteConfirm("");}} style={{width:"100%",background:"rgba(248,113,113,.06)",border:"1px solid rgba(248,113,113,.2)",color:"#f87171",borderRadius:12,padding:"13px",fontFamily:"Inter,sans-serif",fontSize:14,fontWeight:600,cursor:"pointer"}}>
              🗑️ Delete Account
            </button>
          </div>
        )}
      ...
      {showDelete && (
        <div style={{position:"fixed",inset:0,zIndex:1000,background:"rgba(0,0,0,.9)",display:"flex",alignItems:"center",justifyContent:"center",padding:20}}>
          <div style={{background:"#111120",border:`1px solid ${BD}`,borderRadius:18,width:"100%",maxWidth:380,padding:28,animation:"fadeIn .2s ease"}}>

            {deleteStep===1 && <>
              <div style={{textAlign:"center",marginBottom:24}}>
                <div style={{fontSize:52,marginBottom:12}}>⚠️</div>
                <div style={{fontWeight:800,fontSize:20,marginBottom:8,color:"#f87171"}}>Delete Account?</div>
                <div style={{fontSize:13,color:"#888",lineHeight:1.7}}>This will permanently delete:</div>
              </div>
              <div style={{background:"rgba(248,113,113,.06)",border:"1px solid rgba(248,113,113,.15)",borderRadius:10,padding:14,marginBottom:20}}>
                {["Your profile and name","Your watchlist","Your watch history","Your notifications","Your subscription data"].map(item=>(
                  <div key={item} style={{fontSize:12,color:"#f87171",padding:"4px 0",display:"flex",gap:8}}>
                    <span>×</span><span>{item}</span>
                  </div>
                ))}
              </div>
              <div style={{display:"flex",gap:10}}>
                <button onClick={()=>setShowDelete(false)} style={{flex:1,background:"rgba(255,255,255,.06)",border:`1px solid ${BD}`,color:"#aaa",borderRadius:10,padding:"12px",fontSize:13,cursor:"pointer",fontFamily:"Inter,sans-serif"}}>Cancel</button>
                <button onClick={()=>setDeleteStep(2)} style={{flex:1,background:"rgba(248,113,113,.15)",border:"1px solid rgba(248,113,113,.3)",color:"#f87171",borderRadius:10,padding:"12px",fontSize:13,fontWeight:700,cursor:"pointer",fontFamily:"Inter,sans-serif"}}>Continue →</button>
              </div>
            </>}
            ...
            {deleteStep===2 && <>
              <div style={{textAlign:"center",marginBottom:24}}>
                ...
                <div style={{fontSize:13,color:"#888",lineHeight:1.6}}>Type <span style={{color:"#f87171",fontWeight:700,fontFamily:"monospace"}}>delete</span> to permanently delete your account</div>
              </div>
              <input
                className="inp"
                value={deleteConfirm}
                ...
                autoFocus
                style={{marginBottom:16,borderColor:deleteConfirm.toLowerCase()==="delete"?"#f87171":"#1a1a2c"}}
              />
              <div style={{display:"flex",gap:10}}>
                <button onClick={()=>{setShowDelete(false);setDeleteStep(1);setDeleteConfirm("");}} style={{flex:1,background:"rgba(255,255,255,.06)",border:`1px solid ${BD}`,color:"#aaa",borderRadius:10,padding:"12px",fontSize:13,cursor:"pointer",fontFamily:"Inter,sans-serif"}}>Cancel</button>
                <button
                  onClick={deleteAccount}
                  disabled={deleteConfirm.toLowerCase()!=="delete"||deleting}
                  style={{flex:1,background:deleteConfirm.toLowerCase()==="delete"?"#e50914":"rgba(255,255,255,.06)",border:`1px solid ${deleteConfirm.toLowerCase()==="delete"?"#e50914":BD}`,color:"#fff",borderRadius:10,padding:"12px",fontSize:13,fontWeight:700,cursor:deleteConfirm.toLowerCase()==="delete"?"pointer":"not-allowed",fontFamily:"Inter,sans-serif"}}
                >
